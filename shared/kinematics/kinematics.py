...
from shared.arm_model.joint_service import NUM_JOINTS, get_dh_params as _get_dh_params
...
def _dh_transform(dh: DHParameters, theta: float, out: np.ndarray | None = None) -> np.ndarray:
    """Return the 4x4 homogeneous transform for a single DH frame.
...
class D1Kinematics:
    ...
    def forward_kinematics(self, joint_angles: np.ndarray) -> np.ndarray:
        """Compute the 4x4 end-effector pose from joint angles."""
    ...
    def jacobian(self, joint_angles: np.ndarray) -> np.ndarray:
        """Compute the 6xN geometric Jacobian (world frame)."""
        joint_angles = np.asarray(joint_angles, dtype=float)
        T = np.eye(4)
