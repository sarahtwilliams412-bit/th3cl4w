...
class EpisodeAnalyzer:
    ...
    def analyze(
        ...
        self._check_general_criteria(episode, reconstruction, motion_summary, tracking_error, assessment)
    ...
    def _check_general_criteria(
        ...
        assessment.criteria.append(CriterionResult(
            name="no_safety_violations",
            passed=n_safety == 0,
            score=1.0 if n_safety == 0 else 0.0,
            detail=f"{n_safety} safety events during episode" if n_safety else "No safety violations",
        ...
        assessment.criteria.append(CriterionResult(
            ...
            detail=f"{ws_violations} workspace boundary violations" if ws_violations else "Stayed within workspace",
        ...
        assessment.criteria.append(CriterionResult(
            ...
            detail=f"{near_collisions} near-collision events" if near_collisions else "No near-collisions",
        ...
        assessment.criteria.append(CriterionResult(
            ...
            detail=f"Mean tracking error: {np.degrees(mean_err):.2f}°" if mean_err else "No tracking data",
            value=mean_err,
        ...
        cam_connected = any(c.connected for c in episode.camera_refs) if episode.camera_refs else False
        assessment.criteria.append(CriterionResult(
            name="camera_active",
    ...
    def _check_pick_and_place(
        ...
        if reconstruction.n_frames < 2:
            assessment.criteria.append(CriterionResult(
                name="arm_moved", passed=False, score=0.0,
        ...
        ]) if episode.joint_snapshots else np.array([])
    ...
    def _check_go_to_pose(
        ...
        if task.target_position is not None:
            ...
            final_vel = episode.joint_snapshots[-1].velocities[:6] if episode.joint_snapshots else np.zeros(6)
    ...
    def _compute_verdict(self, assessment: EpisodeAssessment) -> None:
        ...
        parts = [f"Verdict: {assessment.verdict.value} (score={assessment.overall_score:.2f}, confidence={assessment.confidence:.2f})"]
        if passed_criteria:
            parts.append(f"Passed: {', '.join(c.name for c in passed_criteria)}")
        if failed_criteria:
            parts.append(f"Failed: {', '.join(c.name + ' (' + c.detail + ')' for c in failed_criteria)}")
