...
class CollisionMemory:
    ...
    def _load(self):
        ...
        try:
            if self.path.exists():
                ...
                logger.info(
                    "Loaded collision memory: %d events, ranges: %s",
                    len(self.events),
                    self.safe_ranges,
                )
    ...
    def record_collision(self, joint_id: int, commanded_deg: float, actual_deg: float):
        ...
        if commanded_deg > 0:
            ...
            logger.warning(
                "J%d: collision at cmd=%.1f° actual=%.1f° → safe max now %.1f°",
                joint_id,
                commanded_deg,
                actual_deg,
                self.safe_ranges[joint_id][1],
            )
        else:
            ...
            logger.warning(
                "J%d: collision at cmd=%.1f° actual=%.1f° → safe min now %.1f°",
                joint_id,
                commanded_deg,
                actual_deg,
                self.safe_ranges[joint_id][0],
            )
