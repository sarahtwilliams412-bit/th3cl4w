...
def setup_logging(
    ...
    *,
    ...
    ----------
    level : int
        Base log level. Overridden to DEBUG when *debug* is True.
    log_file : str | None
        Explicit log file path (legacy). Mutually exclusive with *server_name*.
    fmt : str
    ...
    server_name : str | None
        Short name for this server (e.g. ``"web"``, ``"camera"``). When set,
        logs are written to ``<log_dir>/<server_name>.log`` automatically.
    log_dir : str | Path | None
        Directory for log files. Defaults to ``<project_root>/logs/``.
    debug : bool
        Convenience flag — sets level to DEBUG when True.
    """
    if debug:
        level = logging.DEBUG
