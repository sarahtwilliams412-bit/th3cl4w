...
from shared.kinematics.fk_engine import fk_positions
...
if __name__ == "__main__":
    ...
    for cam_id, r in results.items():
        logger.info(f"\n{cam_id}:")
        logger.info(f"  rvec: {[f'{v:.4f}' for v in r.rvec]}")
        logger.info(f"  tvec: {[f'{v:.4f}' for v in r.tvec]}")
        logger.info(f"  reproj mean: {r.reprojection_error_mean:.2f}px")
        logger.info(f"  reproj max:  {r.reprojection_error_max:.2f}px")
        logger.info(f"  poses used:  {r.num_poses_used}")
        logger.info(f"  inliers:     {r.num_inliers}")
